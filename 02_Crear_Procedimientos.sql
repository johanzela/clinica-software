...
USE [SisCitas]
GO
/****** Object:  StoredProcedure [dbo].[sp_guardarCita]    Script Date: 3/07/2024 00:42:21 ******/
SET ANSI_NULLS ON
GO
...
create procedure [dbo].[sp_guardarCita]
(
@IdUsuario int,
...
as
declare  @fecha date
set @fecha = ( select fecha from DoctorHorarioDetalle where IdDoctorHorarioDetalle = @IdDoctorHorarioDetalle)
declare @hora time
set @hora = (select TurnoHora from DoctorHorarioDetalle where IdDoctorHorarioDetalle = @IdDoctorHorarioDetalle)
...
--agregado por JDiaz el 20240418----
		 if ( exists(select x.Fecha, x.TurnoHora from DoctorHorarioDetalle x join cita y on x.IdDoctorHorarioDetalle = y.IdDoctorHorarioDetalle where y.IdUsuario = @IdUsuario and x.Fecha = @fecha and x.TurnoHora = @hora) )
		begin

			set @msgError = 'Ya tiene una cita para el mismo dia y hora, por favor anularla'
		end
		else
-------
...
begin
	select
	(select count(*) from Doctor)[TotalDoctores],
	(select count(*) from Especialidad)[TotalEspecialidades],
	(select count(*) from cita where IdEstadoCita = 1)[TotalCitaPendientes],
		(select count(*) from cita where IdEstadoCita = 2)[TotalCitaAtendidas],
			(select count(*) from cita where IdEstadoCita = 3)[TotalCitaAnuladas]
